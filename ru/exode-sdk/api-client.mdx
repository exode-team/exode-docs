...
const exodeApi = new ExodeAPI({
  ...
  validate: false,                           // по умолчанию false
})
```
...
<ResponseField name="validate" type="boolean">
  Проверять каждый ответ по zod-схеме (`safeParse`). По умолчанию `false` — payload возвращается как есть
  (типизированный через `z.infer`), без рантайм-парсинга. Можно переопределить на конкретный запрос.
</ResponseField>
...
По умолчанию ответ возвращается как есть (типизированный, без рантайм-проверки). Включите `validate`, чтобы
прогонять payload через zod-схему — глобально или на конкретный запрос:

```ts
// Глобально
const exodeApi = new ExodeAPI({ sellerId: 1, schoolId: 1, token, validate: true })

// На конкретный запрос (low-level request)
await exodeApi.request('GET', 'user/find', {
...
При несоответствии бросается `ExodeAPIError` с `cause: "ValidationError"`.
...
| `ValidationError` | ответ не прошёл zod-валидацию (при `validate: true`) |
