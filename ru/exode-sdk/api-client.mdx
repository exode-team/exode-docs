...
const { items } = await exodeApi.school.user.list({ statuses: ['Active', 'OnLeave'], take: 50 })

// Ban / unban — via status (the `banned` flag was removed from the API)
await exodeApi.school.user.update(user.id, { status: 'Banned' })
await exodeApi.school.user.update(user.id, { status: 'Active' }) // фактический статус пересчитается сам
...
<Tip>
  ...
  [Интеграция с Telegram Mini App](/ru/customization/tg-mini-app).
</Tip>
...
```ts
// Form layouts
const { items: layouts } = await exodeApi.school.form.layoutList({ modes: ['Signup'], take: 20 })
...
_Обновлено: 2026-08-15 16:55 UTC_
