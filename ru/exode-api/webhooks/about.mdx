...
Тестовая отправка из формы **сохранённого** эндпоинта подписывается его же `secretKey`. Формат тела,
...
<Warning>
    У нового, ещё не сохранённого эндпоинта постоянный `secretKey` пока отсутствует. Его тестовая отправка
    подписывается временным ключом, поэтому проверить такую подпись ключом, который появится после создания,
    нельзя. Чтобы проверить полный цикл верификации, сначала сохраните эндпоинт, затем отправьте тест из его
    формы.
</Warning>
...
<Accordion title="PaymentCompleted — оплата прошла успешно">
    ...
    Событие отправляется только при **реальном списании денег**: инициализация рекуррента (привязка карты,
    нулевой платёж со статусом `BindingCompleted`) вебхук не вызывает.
</Accordion>
...
<Accordion title="ProductEnrolledByInviteLink — запись по ссылке-приглашению">
    Триггер: пользователь самостоятельно записался на курс по ссылке-приглашению.

    `data`: те же поля, что и у `ProductEnrolledToFree` (`user`, `profile`, `access`, `product`, `course`),
    плюс:
    - `inviteLinkId` — идентификатор ссылки, по которой пришёл пользователь.

    <Warning>
        При записи по ссылке отправляются **два** события: `ProductEnrolledToFree` и
        `ProductEnrolledByInviteLink`. Ссылка выдаёт доступ бесплатно, поэтому общий контур бесплатных
        зачислений срабатывает тоже — учитывайте это при подсчёте зачислений. Отличить запись по ссылке
        можно по наличию `inviteLinkId`.
    </Warning>
</Accordion>

<Accordion title="CertificateIssued — выдан сертификат">
    Триггер: пользователю выдан сертификат за завершение курса. Событие приходит один раз на пару
    «пользователь + курс»: повторное завершение курса нового сертификата не создаёт.

    `data`:
    - `user` — [пользователь](/ru/exode-api/objects/entities/user) с профилем.
    - `course` — объект [курса](/ru/exode-api/objects/entities/course).
    - `product` — [продукт](/ru/exode-api/objects/entities/product) курса (опционально).
    - `certificate` — объект [сертификата](/ru/exode-api/objects/entities/certificate) с публичной
      ссылкой `link` (открывается без авторизации), сроком действия и слепком данных на момент выдачи.
</Accordion>
</AccordionGroup>
