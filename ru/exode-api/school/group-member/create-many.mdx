...
```
```json Error - Group Not Tied To Product
{
  "code": 400,
  "success": false,
  "cause": "GroupNotBindToProduct",
...
<ResponseField name="exist" type="array">
    ...
    <Expandable title="Свойства участника">
        ...
        <ResponseField name="blockedUntil" type="timestamp | null">
            Дата, до которой участник заблокирован (если применимо).
        </ResponseField>

        <ResponseField name="isAddedToTg" type="boolean | null">
            Добавлен ли участник в Telegram-чат/канал группы.
        </ResponseField>
        ...
        <ResponseField name="user" type="object">
            ...
            <Expandable title="Свойства пользователя">
                ...
                <ResponseField name="status" type="enum">
                    Статус пользователя: `Active`, `OnLeave`, `Banned`, `Blocked`, `Terminated`, `Deleted`.
                </ResponseField>

                <ResponseField name="active" type="boolean">
                    Не удалён (производное от `status`).
                </ResponseField>

                <ResponseField name="activated" type="boolean">
                    Подтвердил ли пользователь вход.
                </ResponseField>

                <ResponseField name="banned" type="boolean">
                    Забанен или удалён (производное от `status`).
                </ResponseField>

                <ResponseField name="domain" type="string">
                    Домен регистрации пользователя.
                </ResponseField>
                ...
                <ResponseField name="timezone" type="integer | null">
                    Часовой пояс пользователя (смещение).
                </ResponseField>
...
<ResponseField name="created" type="array">
    Массив пользователей, которые были добавлены в группу. Каждый элемент имеет ту же структуру, что и элементы
    массива `exist`.
</ResponseField>

<ResponseField name="excluded" type="array">
    Массив пользователей, пропущенных из-за исключения из автоматического назначения. Элементы — объекты
    пользователя (та же структура, что и вложенный `user` в элементах `exist`).

    Для этого метода массив **всегда пустой**: исключения ограничивают только автоматическое назначение по
    правилам оргструктуры, а добавление через API считается ручным. Поле присутствует в ответе всегда — учитывайте
    его при строгой валидации схемы.
</ResponseField>

<Note>
    Ручное добавление **снимает** ранее выставленное исключение: если пользователя до этого убрали из группы
    вручную, после добавления через API правила автоматического назначения снова смогут его назначать.
</Note>
