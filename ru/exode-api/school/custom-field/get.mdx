...
<ParamField query="page" type="integer" required={false}>
  Номер страницы (от `1`). Альтернатива `skip` — смещение вычисляется автоматически как `(page - 1) * take`.
</ParamField>
...
<ParamField query="order" type="enum" required={false}>
  Сортировка по порядковому номеру поля. Возможные значения: `ASC`, `DESC`.
</ParamField>
...
<Info>
  ...
  - `field` — вложенный объект с информацией о поле (`id`, `slug`, `type`, `order`, `layoutId`, `props`, `preference`, `permissions`)
  - `userId`, `fieldId` — идентификаторы для связи
  - типизированные столбцы (`text`, `number`, `boolean`, `date`, `json`) также присутствуют в ответе в зависимости от типа поля; удобное поле `value` содержит уже приведённое значение
</Info>
