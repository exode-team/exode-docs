...
<Info>
  Для режимов `Signup` и `Welcome` допускается только один макет на продавца (школу). При попытке создать второй макет с тем же режимом будет возвращена ошибка `LayoutWithModeAlreadyExists`.
</Info>
...
```
```json Error - Duplicate Mode (Signup / Welcome)
{
  "code": 400,
  "success": false,
