...
У сотрудника не может быть активного дубля с той же парой департамент + должность. Если пользователь ранее был
уволен и имеет статус `Terminated`, найм автоматически возвращает ему статус `Active`.

Должность и департамент указываются либо по ID, либо по внешнему идентификатору: в каждой из пар
`positionId` / `positionExtId` и `departmentId` / `departmentExtId` передаётся ровно один параметр.
...
<ParamField body="positionId" type="integer" required={false}>
    ID должности. Должность должна принадлежать школе. Передаётся ровно один параметр из пары
    `positionId` / `positionExtId`.
</ParamField>
...
`extId` в рамках школы. Тело запроса — как у обычной смены должности, без поля `employmentId`.
