...
<Info>
    ...
    вход и доступ к платформе он **не** блокирует. Переходы не затрагивают пользователей в других статусах
    (`Banned`, `Blocked`, `Terminated`). Помимо пересчёта при API-вызовах, платформа раз в час автоматически
    сверяет статусы с календарём: отсутствие с будущей датой начала переведёт сотрудника в `OnLeave` при её
    наступлении, а истёкшее — вернёт `Active` без каких-либо вызовов с вашей стороны.
</Info>
