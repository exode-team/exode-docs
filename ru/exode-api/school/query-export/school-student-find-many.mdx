...
description: "Выгрузка студентов школы с профилем, статусами и активностью"
---

**type:** `QUERY_EXPORT_TYPE_SCHOOL_STUDENT_FIND_MANY`

Выгружает список студентов школы (пользователей с ролью `Student`) с данными профиля, статусами доступа и
активностью.
...
<Info>
  `variables` имеют ту же структуру `{ filter, sort }`, что и отчёт
  [«Пользователи школы»](/ru/exode-api/school/query-export/school-user-find-many), с выборкой, ограниченной
  студентами. Итоговый набор допустимых полей валидирует сервис экспорта — неизвестные поля игнорируются.
</Info>
...
<ParamField body="filter" type="object">
  ...
  <Expandable title="Свойства filter">
    ...
    <ParamField body="active" type="boolean">
      Только активные (`true`) или отключённые (`false`).
    </ParamField>
