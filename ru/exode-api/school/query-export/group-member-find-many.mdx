...
Дополнительный лист **Course Progress** — детализация прогресса по курсам (включая вложенные курсы бандла):
...
<ParamField body="filter" type="object" required>
  ...
  <Expandable title="Свойства filter">
    ...
    <ParamField body="product" type="object">
      Вложенный фильтр по продукту. Набор полей совпадает с фильтром `product` метода
      [«Список доступов к продуктам»](/ru/exode-api/school/product-access/list).
    </ParamField>

    <ParamField body="price" type="object">
      Вложенный фильтр по тарифу (цене) продукта. Набор полей совпадает с фильтром `price` метода
      [«Список доступов к продуктам»](/ru/exode-api/school/product-access/list).
    </ParamField>
  </Expandable>
</ParamField>
