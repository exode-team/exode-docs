...
<ParamField body="extId" type="string" required={false}>
  Внешний идентификатор пользователя из вашей системы (например, GUID из CRM/1С). Строка или `null`.
  Логином не является — используется только для связи и поиска.
</ParamField>
...
<ParamField body="status" type="enum" required={false}>
  ...
  [отсутствий](/ru/exode-api/school/staff/absence)); `Blocked` и `Terminated` закрывают доступ: вход запрещён,
  активные сессии завершаются, зачисления блокируются. `Terminated` управляется автоматически модулем
  [трудоустройств](/ru/exode-api/school/staff/employment). Подробнее — в
  [`user/create`](/ru/exode-api/school/user/create).
</ParamField>

<Warning>
  Владелец школы защищён: если апсерт находит владельца, перевод его в `banned: true` или блокирующий статус
  (`Blocked`/`Terminated`) запрещён — вернётся ошибка `ForbiddenModifySchoolOwner`.
</Warning>
