...
<Tip>
    **Upsert** — это операция "создать или обновить". Если пользователь с указанным email, телефоном или Telegram ID уже
    существует,
    то его данные будут обновлены. Если пользователя нет — он будет создан.
</Tip>
...
const upsertUser = async () => {
  try {
    ...
    console.log('Is created:', response.data.isCreated);
  } catch (error) {
...
if ($httpCode === 200) {
  ...
  echo "Is created: " . ($result['isCreated'] ? 'true' : 'false') . "\n";
  print_r($result);
} else {
...
try:
  ...
  print(f"Is created: {result['isCreated']}")
