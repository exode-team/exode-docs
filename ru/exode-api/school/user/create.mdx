...
<ParamField body="profile" type="object" required={false}>
  ...
  <Expandable title="Свойства профиля">
    <ParamField body="firstName" type="string" required={false}>
      Имя пользователя. Максимум 15 символов: буквы (латиница и кириллица, включая ў/ғ/қ/ҳ), пробелы и апострофы
      (например, `O'Brien`). Автоматически обрезаются пробелы в начале и конце.
    </ParamField>

    <ParamField body="lastName" type="string" required={false}>
      Фамилия пользователя. Максимум 15 символов: буквы (латиница и кириллица, включая ў/ғ/қ/ҳ), пробелы и апострофы
      (например, `O'Brien`). Автоматически обрезаются пробелы в начале и конце.
    </ParamField>
      ...
      **обязателен** — сотрудник не может существовать без трудоустройства.
      ...
      <Expandable title="Свойства staff">
        <ParamField body="employments" type="array" required>
          ...
          Департамент в каждом элементе **обязателен** и указывается через `departmentId` **или**
          `departmentExtId`. Должность **необязательна**: её можно задать через `positionId` **или**
          `positionExtId`, либо не передавать вовсе — тогда сотрудник нанимается без должности (внешние
          идентификаторы из вашей системы, см. разделы
          ...
          <Expandable title="Свойства элемента employments">
            ...
            <ParamField body="positionId" type="integer" required={false}>
              ID должности в Exode. Необязателен. Указывается через `positionId` **или** `positionExtId` (не оба
              одновременно), либо не передаётся вовсе — тогда сотрудник нанимается без должности.
            </ParamField>

            <ParamField body="positionExtId" type="string" required={false}>
              Внешний идентификатор должности из вашей системы. От 1 до 50 символов. Необязателен — альтернатива
              `positionId` (передаётся не более одного поля из пары).
            </ParamField>
