...
<ParamField body="extId" type="string" required={false}>
  Внешний идентификатор пользователя из вашей системы (например, GUID из CRM/1С). Строка до 50 символов или `null`.
  При передаче пустой строки — преобразуется в `null`. Используется для связи и поиска пользователя
  (`user/find`, `user/upsert`), **для входа не используется**.
</ParamField>
...
<ParamField body="status" type="enum" required={false}>
  Статус учётной записи: `Active`, `OnLeave`, `Blocked` или `Terminated`. По умолчанию `Active`.
