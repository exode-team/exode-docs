...
<ParamField body="domain" type="string" required={false}>
  Доменный логин пользователя. До 65 символов: латинские буквы, цифры и `_` (автоматически приводится к нижнему
  регистру). Должен быть уникальным в рамках школы. Если не передан — генерируется автоматически в формате
  `id12345`. Удобен, когда логины формируются на вашей стороне и у пользователя нет email или телефона.
</ParamField>
