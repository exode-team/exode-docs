...
description: "Поиск пользователя в школе по логину или Telegram ID"
---
...
<Info>
  Поиск осуществляется по логину (email или телефон) или Telegram ID. Если пользователь не найден — возвращается `null`.
</Info>
...
<Warning>
  Необходимо передать **хотя бы один** из параметров. Если передать оба — приоритет отдается `login`.
</Warning>

<RequestExample>
```bash cURL
curl --location --request GET 'https://api.exode.biz/saas/v2/user/find?login=user@example.com' \
