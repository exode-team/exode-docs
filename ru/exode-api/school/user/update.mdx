...
<ParamField body="status" type="enum" required={false}>
  ...
  - `Banned` — забанен: вход запрещён, активные сессии завершаются. Чтобы снять бан — передайте `Active`:
    фактический статус пересчитается автоматически по трудоустройствам и отсутствиям (`Terminated` / `OnLeave` /
    `Active`), поэтому уволенный до бана сотрудник не получит доступ по ошибке;
...
<Warning>
  Владелец школы защищён: другой администратор не может изменить его данные, а перевод владельца в блокирующий
  статус (`Banned`/`Blocked`/`Terminated`) запрещён даже для него самого — вернётся ошибка
