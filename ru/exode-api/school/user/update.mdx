...
<ParamField body="extId" type="string" required={false}>
  Внешний идентификатор пользователя из вашей системы. Строка или `null`. Логином не является — используется только
  для связи с вашей CRM/LMS и поиска пользователя.
</ParamField>
...
<Info>
  Блок `extra` (трудоустройства) в `update` **не применяется** — для изменения трудоустройств существующего
  сотрудника используйте эндпоинты раздела [Сотрудники (HR)](/ru/exode-api/school/staff/employment) или
