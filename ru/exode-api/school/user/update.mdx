...
<ParamField path="extId" type="string" required>
  Внешний идентификатор пользователя в рамках школы. Значение в пути должно быть **URL-encoded**
  (`encodeURIComponent`): у пользователя `extId` может содержать любые символы, включая `/` и пробелы.
</ParamField>
