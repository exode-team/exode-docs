...
<ParamField query="courseIds" type="integer[]" required={false}>
    Фильтр по ID курсов: доступы к продуктам, которым принадлежат указанные курсы.
</ParamField>
...
<ParamField query="billingActive" type="boolean" required={false}>
    Только с активным (`true`) биллингом. «Активный» — продление не отменено **и** первая оплата уже прошла
    (доступы в ожидании входного платежа, `WaitingEntryFee`, не считаются активными).
</ParamField>
...
<ParamField query="user" type="object" required={false}>
    ...
    <Expandable title="Свойства user">
        ...
        <ParamField query="extIds" type="string[]">Внешние идентификаторы (до 250 значений, каждый до 50 символов).</ParamField>
        ...
        <ParamField query="statuses" type="enum[]">
            Статусы учётной записи: `Active`, `OnLeave`, `Banned`, `Blocked`, `Terminated`, `Deleted`
            (поля `active`/`banned` из фильтра удалены — используйте `statuses`).
        </ParamField>
