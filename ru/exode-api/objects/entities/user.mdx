...
<ResponseField name="status" type="enum" required>
    Статус учётной записи: `Active`, `OnLeave`, `Blocked`, `Terminated`. `Active` — обычный доступ; `OnLeave` —
    «в отсутствии» (информационный, доступ не блокирует; синхронизируется автоматически с отсутствиями сотрудника);
    `Blocked` — заблокирован администратором; `Terminated` — уволен (ставится автоматически при увольнении с
    последнего активного трудоустройства, повторный найм возвращает `Active`). `Blocked` и `Terminated` закрывают
    доступ: вход запрещён, сессии завершаются, зачисления на продукты блокируются.
</ResponseField>
<ResponseField name="alive" type="boolean | null">
    «Живой» доступ: `active === true`, `banned === false` и `status` не входит в `Blocked`/`Terminated`.
</ResponseField>
